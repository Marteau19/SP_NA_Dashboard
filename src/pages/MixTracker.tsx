...
import { Card, PageHeader, SectionTitle, MixBullet } from "../components/common/ui";
    ...
    <div>
        ...
        action={
          <div className="w-60">
            <div className="mb-1 flex justify-between text-xs text-stone-500">
              <span>Network service mix</span>
              <span className="tabular-nums">{gap >= 0 ? "+" : ""}{gap.toFixed(1)} pts</span>
            </div>
            <MixBullet value={totals.servicePct} />
          </div>
        }
      />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="flex flex-col justify-center p-6">
          <span className="text-sm font-medium text-stone-500">Network service mix</span>
          <div className="mt-2 flex items-baseline gap-3">
            <span className="text-5xl font-extrabold text-navy tabular-nums">{pct(totals.servicePct)}</span>
            <span className="text-lg font-semibold text-stone-400 tabular-nums">/ {SERVICE_TARGET}%</span>
          </div>
          <div className="mt-4"><MixBullet value={totals.servicePct} showValue={false} /></div>
          <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
            <div className="rounded-lg border border-stone-200 p-3">
              <p className="text-xs text-stone-500">Leading</p>
              <p className="text-2xl font-extrabold text-navy tabular-nums">{leaders.length}</p>
              <p className="text-xs text-stone-500">SPs at / above {SERVICE_TARGET}%</p>
            </div>
            <div className="rounded-lg border border-stone-200 p-3">
              <p className="text-xs text-stone-500">Lagging</p>
              <p className="text-2xl font-extrabold text-navy tabular-nums">{laggards.length}</p>
              <p className="text-xs text-stone-500">SPs below {SERVICE_TARGET}%</p>
            </div>
          </div>
        </Card>
